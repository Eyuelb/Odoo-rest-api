          ...
          <div className="grid grid-cols-1  md:grid-cols-2 lg:grid-cols-3 gap-2 lg:gap-4">
            {products?.map((product) => (
              ...
              >
                ...
                <p className='font-bold py-2 bg-gray-200 text-center rounded text-base'> Ksh {product?.listPrice}</p>
              </div>
            ))}
          ...
          <tbody>
            {cart?.map((item) => (
              <tr className="text-xs font-medium text-black border-b border-black" key={item.name}>
                ...
                <td className='text-end px-2'>{item.productQuantity}</td>
                <td className='capitalize text-end px-3'>{formatCurrency.format(item.listPrice)}</td>
              </tr>
            ))}
